...
def arg_parser(prompt=prompt):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="whether to use azure openai api, default is False, use openai api",
    )
...
def load_metadata(args):
    ...
    mmvet_metadata = os.path.join(args.mmvetv2_path, "mm-vet.json")
                    ...
                    try:
                        ...
                        while flag:
                            try:
                                ...
                                try_time += 1
                                temperature += 0.5
                                print(f"{id} try {try_time} times, content: {content}")
                                if try_time > 5:
                                    score = 0.0
                                    flag = False
...
def export_result(
    args, model, df, df2, grade_results, data, cap_set_counter, cap_set_names
):
    columns = df.columns
...
if __name__ == "__main__":
    ...
    if args.azure:
        ...
        args.gpt_model = (
            "gpt-4-32k-0613"  # for our azure, the model can only be gpt-4-32k-0613
        )
    else:
    ...
    print("\n")
    print(
        f"Grading results are saved in:\n{grade_file}\n{cap_score_file}\n{cap_int_score_file}"
    )
