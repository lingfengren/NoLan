    ...
    def generate(
        ...
        if image.dim() == 5:
            ...
            if images_cd is not None:
                ...
                image_atts_cd = torch.ones(
                    image_embeds_cd.size()[:-1], dtype=torch.long
                ).to(image.device)
            ...
            if images_cd is not None:
                inputs_llm_cd = self.llm_proj(
                    query_output_cd.last_hidden_state[:, : query_tokens.size(1), :]
                )
                atts_llm_cd = torch.ones(
                    inputs_llm_cd.size()[:-1], dtype=torch.long
                ).to(image.device)
        ...
        with self.maybe_autocast():
            ...
            # llm_tokens.input_ids = llm_tokens.input_ids.to(device)
            # inputs_llm = inputs_llm.to(device)
            # end multi gpu
            inputs_embeds = self.llm_model.get_input_embeddings()(llm_tokens.input_ids)
            inputs_embeds = torch.cat([inputs_llm, inputs_embeds], dim=1)
            attention_mask = torch.cat([atts_llm, llm_tokens.attention_mask], dim=1)
            if input_ids_cd is not None:
                inputs_embeds_cd = self.llm_model.get_input_embeddings()(
                    llm_tokens.input_ids
                )
                attention_mask_cd = llm_tokens.attention_mask
            else:
                inputs_embeds_cd = None
            ## cd_comments: format
            outputs = self.llm_model.generate(
                inputs_embeds=inputs_embeds,
                input_ids_cd=inputs_embeds_cd,
                attention_mask=attention_mask,
